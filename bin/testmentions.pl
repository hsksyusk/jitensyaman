...
#					print "data_line=" . $data_line . "\n";
				...
				if ($data_line =~ s/(?:最高|最大|MAX).{0,3}?(?:ケイデンス|cadence).{0,3}?(\d+\.?\d?).{0,3}?(rpm)?//i ){
...
#					print "data_line=" . $data_line . "\n";
				...
				if ($data_line =~ s/(?:ケイデンス|cadence).{0,3}?(\d+\.?\d?).{0,3}?(rpm)?//i ){
...
#			}
			...
			if ( defined $track{'distance'} or defined $track{'time'} or defined $track{'elevation'} 
				...
				if ( defined $track{'distance'} && $calc_flag{'distance'} ){
					...
					$tweet_message = $tweet_message . "今日の走行記録";
				}
...
#				$twitter->update($tweet_message);
				...
				if ( defined $track{'date'} ) {
					...
					$tweet_message = $tweet_message . "これまでの全て";
				}
				$tweet_message = $tweet_message . "の走行記録は";
				...
				if ( $mytracks_count > 0 ){
					...
					$tweet_message = $tweet_message . "、この期間の走行記録は聞いてないぞ！";
				}
#				$twitter->update($tweet_message);
				print $tweet_message . "\n";
			} else {
#				$twitter->update($tweet_message . "自転車マンだ！");
				print $tweet_message . "自転車マンだ！\n";
			}
		}
	}
	$page++;
